          ...
          <div v-for="order in orderList" :key="order.id" class="order-item">
            <div class="head">
              ...
              <span v-if="order.orderState === 1" class="down-time">
                ...
                <b>
                  ...
                  {{
                    order.countdown !== -1
                      ? '剩余' + dayjs.unix(order.countdown ?? 0).format('mm分ss秒')
                      : '已截止'
                  }}
                </b>
              </span>
            ...
            <div class="body">
              ...
              <div class="column action">
                <el-button
                  v-if="order.orderState === OrderState.UNPAID"
                  :disabled="order.countdown === -1"
                  size="small"
                  type="primary"
                >
                  立即付款
...
import { OrderState, OrderStateMap } from '@/constants/order'
...
const tabTypes = Object.entries(OrderStateMap).map(([value, label]) => ({
  name: Number(value) as OrderStateValue,
  label,
}))
const total = ref<number>(0)
const orderList = ref<MyOrderVO[]>([])
const params = ref<GetUserOrderParams>({
...
const handleClick = (name: TabPaneName) => {
  params.value.orderState = name as OrderStateValue
  params.value.page = 1
  getOrderList()
}
